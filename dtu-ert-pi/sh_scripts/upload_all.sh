...
$USRBIN/rsync -rtlDz --timeout=300 -e "ssh -i $SSHKEY" --exclude "/.*" --chmod "Da=rw,Fa=rw" /root/ $SSHUSER@$SERVER_IP:$UPLDESTDIR/home_root/ 2>> $LOGDIR/uploadlog
echo "Uploading logs..."
$USRBIN/rsync -rtlDz --timeout=300 -e "ssh -i $SSHKEY" --exclude "/.*" --chmod "Da=rw,Fa=rw" $LOGDIR/ $SSHUSER@$SERVER_IP:$UPLDESTDIR/logs/ 2>> $LOGDIR/uploadlog
echo "Uploading crontab backups..."
$USRBIN/rsync -rtlDz --timeout=300 -e "ssh -i $SSHKEY" --exclude "/.*" --chmod "Da=rw,Fa=rw" $CRONTABDIR/ $SSHUSER@$SERVER_IP:$UPLDESTDIR/crontabs/ 2>> $LOGDIR/uploadlog
echo "Uploading crontab backups..."
