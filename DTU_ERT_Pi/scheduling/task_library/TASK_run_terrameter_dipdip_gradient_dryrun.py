...
@add_task
def run_terrameter_dipdip_dryrun(*args, boot_delay=None, **kwargs):
    power_off = False
    cmd_file = 'command_2x32_dipdip_gradient_ecr_1sec'

    if boot_delay is not None:
        terrameter.wake_and_run(cmd_file, boot_delay=boot_delay, dryrun=True, power_off=power_off)
    else:
        terrameter.wake_and_run(cmd_file, dryrun=True, power_off=power_off)


if __name__ == '__main__':
    if len(sys.argv) > 1:
        run_terrameter_dipdip_dryrun(boot_delay=int(sys.argv[1]))
    else:
        run_terrameter_dipdip_dryrun()
