...
@add_task
def run_terrameter_no_command(*args, boot_delay=None, **kwargs):
    cmd_file = None
    power_off = False
    
    terrameter.wake_and_run(cmd_file, boot_delay=boot_delay, power_off=power_off)
    terrameter.log_to_file('RUN: Terrameter is still ON, turn off manually!', echo=True)
    
if __name__ == '__main__':
    if len(sys.argv) > 1:
        run_terrameter_no_command_2(boot_delay=int(sys.argv[1]))
    else:
        run_terrameter_no_command_2()