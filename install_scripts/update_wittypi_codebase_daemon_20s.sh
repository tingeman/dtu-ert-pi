...
WITTYPI_DOWNLOAD_URL="https://github.com/tingeman/Witty-Pi-4/archive/952cc0871fe00b6efa2b33312bb109b78d3e8d8b.zip"
...
elif [[ $WITTYPI_GIT_BRANCH == commit ]]; then
  wget "$WITTYPI_DOWNLOAD_URL"/main.zip -O "$TMP_DIR/wittyPi.zip"
  match=$(echo $WITTYPI_DOWNLOAD_URL | grep '/refs/heads')
  if [[ -z $match ]]; then
    SRC_DIR="$TMP_DIR"/"Witty-Pi-4-"$(basename $WITTYPI_DOWNLOAD_URL .zip)
  else
    branch_ref=$(echo $WITTYPI_DOWNLOAD_URL | sed 's#.*/refs/heads/\(.*\)#\1#' | sed 's#/#-#' | sed 's/.zip//')
    SRC_DIR="$TMP_DIR"/"Witty-Pi-4-"$branch_ref
  fi
else
